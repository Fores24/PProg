...
void graphic_engine_paint_game(Graphic_engine *ge, Game *game) {
    Id id_act = NO_ID, id_back = NO_ID, id_next = NO_ID, obj_loc = NO_ID, id_oca_east = NO_ID, id_oca_west = NO_ID;
    ...
    if ((id_act = game_get_player_location(game)) != NO_ID) {
        ...
        id_oca_east = space_get_east(space_act);
        id_oca_west = space_get_west(space_act);

        obj = ' ';
        for (i = 0; i < MAX_OBJECT && game->object[i] != NULL; i++) { /*Miramos todos los objetos del juego*/
            if (game_get_object_location(game, object_get_id(game->object[i])) == id_back)
                obj = '*';
        }
        if (id_back != NO_ID) {
        ...
        obj = ' ';
        for (i = 0; i < MAX_OBJECT && game->object[i] != NULL; i++) { /*Miramos todos los objetos del juego*/
            if (game_get_object_location(game, object_get_id(game->object[i])) == id_act)
                obj = '*';
        }
        ...
        if (id_act != NO_ID) {
            ...
            screen_area_puts(ge->map, str);
            if (id_oca_east != NO_ID && id_oca_west != NO_ID) {
                sprintf(str, "%ld<|     %c     |>%ld", id_oca_west, obj, id_oca_east);
            } else if (id_oca_east != NO_ID) {
                sprintf(str, "  |     %c     |>%ld", obj, id_oca_east);
            } else if (id_oca_west != NO_ID) {
                sprintf(str, "%ld<|     %c     |", id_oca_west, obj);
            } else {
                sprintf(str, "  |     %c     |", obj);
            }
            screen_area_puts(ge->map, str);
        ...
        obj = ' ';
        for (i = 0; i < MAX_OBJECT && game->object[i] != NULL; i++) { /*Miramos todos los objetos del juego*/
            if (game_get_object_location(game, object_get_id(game->object[i])) == id_next)
                obj = '*';
        }
    ...
    for (i = 0; i < MAX_OBJECT && game->object[i] != NULL; i++) { /*Miramos todos los objetos del juego*/
        if ((obj_loc = game_get_object_location(game, object_get_id(game->object[i]))) != NO_ID) {
            sprintf(str, "  Object location:%d %s", (int) obj_loc, game_get_object_name(game, object_get_id(game->object[i])));
    ...
    sprintf(str, "     following or f, previous or p, oca_following or o, oca_previous or q, coger or c, soltar or s, or exit or e");
    screen_area_puts(ge->help, str);
    ...
    screen_paint();
    printf("J1: ");
    print_player(stdout, game->player); /*Escribimos los datos del jugador*/
    printf("\nprompt:> ");
}
